...
port-check: generate-core
	...
	$(PYTHON) -I tools/core_top_gate.py port-check --timeout $(CORE_TOP_GATE_TIMEOUT) --repo-root . --manifest reference/manifest.lock --ports reference/core-top.ports.json --rtl "$(CORE_TOP_RTL)" --out-dir "$(OUT_DIR)/core_top/port-check"

lint: generate-core
	rm -rf "$(OUT_DIR)/core_top/lint"
	$(PYTHON) -I tools/core_top_gate.py lint --timeout $(CORE_TOP_GATE_TIMEOUT) --repo-root . --manifest reference/manifest.lock --ports reference/core-top.ports.json --rtl "$(CORE_TOP_RTL)" --out-dir "$(OUT_DIR)/core_top/lint" --environment-profile "$(CORE_TOP_LINT_PROFILE)" $(if $(CORE_TOP_LINT_WAIVERS),--waivers "$(CORE_TOP_LINT_WAIVERS)",)

yosys-check: generate-core
	rm -rf "$(OUT_DIR)/core_top/yosys-check"
	$(PYTHON) -I tools/core_top_gate.py yosys-check --timeout $(CORE_TOP_GATE_TIMEOUT) --repo-root . --manifest reference/manifest.lock --ports reference/core-top.ports.json --rtl "$(CORE_TOP_RTL)" --out-dir "$(OUT_DIR)/core_top/yosys-check"

publish-check: generate-core
	rm -rf "$(OUT_DIR)/core_top/publish-check"
	$(PYTHON) -I tools/core_top_gate.py publish-check --timeout $(CORE_TOP_GATE_TIMEOUT) --repo-root . --manifest reference/manifest.lock --ports reference/core-top.ports.json --rtl "$(CORE_TOP_RTL)" --tracked-rtl rtl/mycpu_top.v --replacement-spec reference/component-replacements/core-top.json --out-dir "$(OUT_DIR)/core_top/publish-check"
