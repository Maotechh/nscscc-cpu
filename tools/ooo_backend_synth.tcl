if {$argc != 2} {
  puts stderr "usage: vivado -mode batch -source tools/ooo_backend_synth.tcl -tclargs <rtl> <out-dir>"
  exit 2
}
...
read_verilog -sv $rtl
synth_design -top ooo_backend_with_execution -part xc7a200tfbg676-2 -flatten_hierarchy rebuilt
...
write_checkpoint -force [file join $out_dir ooo_backend_synth.dcp]

set worst_slack [get_property SLACK [get_timing_paths -delay_type max -max_paths 1]]
puts "OOO_SYNTH_WNS=$worst_slack"
puts "OOO_SYNTH_OUT=$out_dir"
exit 0
