...
// ALU operation codes (matching openLA500 alu.v)
object AluOp {
...
case class ALU() extends Component {
  ...
  val sub_result = io.src1.asSInt - io.src2.asSInt

  // ALU result multiplexer
  io.result := io.src1
  when(io.alu_op === ADD)   { io.result := io.src1 + io.src2 }
    .elsewhen(io.alu_op === SUB)   { io.result := (io.src1 - io.src2) }
    .elsewhen(io.alu_op === SLT)   { io.result := ((io.src1.asSInt < io.src2.asSInt) ? U(1, 32 bits) | U(0, 32 bits)) }
    .elsewhen(io.alu_op === SLTU)  { io.result := ((io.src1 < io.src2) ? U(1, 32 bits) | U(0, 32 bits)) }
