...
import openla500.observe.{ArchState, ChiplabMultiCommitDiffTestAdapter, CommitEvent}
...
final class OooCoreSystem(
  ...
  val architecturalGpr = Vec.fill(config.archRegs)(Reg(Bits(config.xlen bits)) init (B(0, config.xlen bits)))
  architecturalGpr(0) := B(0, config.xlen bits)
  for (lane <- 0 until config.commitWidth) {
    when(core.io.commitValid(lane) && core.io.commit(lane).retired &&
  ...
  io.debugGprWriteMask := B(4 bits, default ->
