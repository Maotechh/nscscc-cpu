...
case class IDStage() extends Component {
  ...
  val is_load      = opcode(5 downto 2) === U(0xA, 4 bits) // 0x22-0x2F → (5..2)=10,9,8 not quite right, simplified
...
case class EXEStage() extends Component {
  ...
  val br_taken = id.branch && (id.branch_op.mux(
    0 -> !br_eq, 1 -> br_eq, 2 -> br_lt, 3 -> br_ltu, default -> False))
  ...
  io.exe2mem.except_code := id.excp_ine ? U(0x0D, 6 bits) | (id.excp_sys ? U(0x0B, 6 bits) | U(0, 6 bits))
...
case class MEMStage() extends Component {
  ...
  val bdata = alow.mux(0 -> io.dmem_rdata(7 downto 0), 1 -> io.dmem_rdata(15 downto 8),
                        2 -> io.dmem_rdata(23 downto 16), default -> io.dmem_rdata(31 downto 24))
