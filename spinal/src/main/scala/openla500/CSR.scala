...
case class CSRFile() extends Component {
  val io = new Bundle {
    ...
    val csr_num = in UInt (14 bits); val csr_wdata = in UInt (32 bits);
    val csr_rdata = out UInt (32 bits)
    val excp_valid = in Bool (); val excp_era = in UInt (32 bits)
    val excp_code = in UInt (6 bits); val excp_subcode = in UInt (9 bits);
    val excp_badv = in UInt (32 bits)
    val ertn_valid = in Bool ()
    val in_kernel = out Bool (); val da_mod = out Bool (); val pg_mod = out Bool ();
    val user_mode = out Bool ()
  ...
  val crmd_plv = Reg(UInt(2 bits)) init (0); val crmd_da = Reg(Bool()) init (True);
  val crmd_pg = Reg(Bool()) init (False)
  ...
  when(io.csr_rd) {
    switch(io.csr_num) {
      is(CRMD) {
        io.csr_rdata := crmd_plv.resize(32) | (crmd_da.asUInt(32 bits) |<< 3) | (crmd_pg.asUInt(
          32 bits
        ) |<< 4)
      }
