...
class CPUCoreFlat extends Component {
  ...
  val id_pc    = Reg(UInt(32 bits)); val id_inst = Reg(UInt(32 bits)); val id_valid = Reg(Bool()) init(False)
  ...
  val exe_mem_wdata  = Reg(UInt(32 bits)); val exe_mem_read  = Reg(Bool()); val exe_mem_write = Reg(Bool())
  val exe_mem_size   = Reg(UInt(2 bits)); val exe_mem_sext = Reg(Bool()); val exe_wb_en = Reg(Bool())
  val exe_valid = Reg(Bool()) init(False)
  val mem_result = Reg(UInt(32 bits)); val mem_wb_en = Reg(Bool()); val mem_valid = Reg(Bool()) init(False)
  ...
  .elsewhen(is_mem || is_lu12iw || is_pcaddu12i || is_jirl || is_jump || is_branch) { alu_op := ADD }
  ...
  when(id_valid && !stall_id) {
    ...
    id_mem_read := is_load; id_mem_write := is_store; id_mem_size := Mux(is_mem, opcode(1 downto 0), U(2, 2 bits))
    id_mem_sext := !opcode(2); id_wb_en := is_alu_reg || is_alu_imm || is_load || is_jirl || is_jump || is_lu12iw || is_pcaddu12i
  ...
  // Branch resolution
  val br_taken = id_branch && (
    (id_branch_op === 0 && (exe_src1 =/= exe_src2)) ||
  ...
  val dc_rd_data = dc_data(((Mux(dc_w0, U(0, 1 bits), U(1, 1 bits))) @@ dc_idx @@ dc_off).resize(11 bits))
  ...
  val bdata = exe_mem_addr(1 downto 0).mux(
    0 -> dc_rd_data(7 downto 0), 1 -> dc_rd_data(15 downto 8),
    2 -> dc_rd_data(23 downto 16), default -> dc_rd_data(31 downto 24))
  ...
  val load_result = exe_mem_size.mux(
    ...
    0 -> (exe_mem_sext ? bdata.asSInt.resize(32).asUInt | bdata.resize(32)), default -> U(0, 32 bits))
  ...
  io.wid := 0; io.wdata := 0; io.wstrb := 0; io.wlast := True; io.wvalid := False; io.bready := False
