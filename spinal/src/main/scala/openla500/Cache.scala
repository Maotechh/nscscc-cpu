...
case class ICache() extends Component {
  val io = new Bundle {
    ...
    val req_addr = in UInt (32 bits); val req_wdata = in UInt (32 bits);
    val req_wstrb = in UInt (4 bits)
    val rsp_valid = out Bool (); val rsp_data = out UInt (32 bits); val cache_miss = out Bool ()
    val cacop_en = in Bool (); val cacop_mode = in UInt (2 bits);
    val cacop_vaddr = in UInt (32 bits)
  ...
  val st = RegInit(IDLE)
  val ma = Reg(UInt(32 bits)); val mw = Reg(UInt(1 bits)); val mr = Reg(UInt(2 bits));
  val mi = Reg(UInt(ICACHE_INDEX bits))

  val cap = rbc
  val c0 = cap && rbcm === U(0, 2 bits);
  val c1 = cap && (rbcm === U(1, 2 bits) || rbcm === U(3, 2 bits))
...
case class DCache() extends Component {
  val io = new Bundle {
    ...
    val refill_valid = in Bool (); val refill_data = in UInt (32 bits)
    val refill_last = in Bool (); val refill_ready = out Bool ()
    val wb_req = out Bool (); val wb_addr = out UInt (32 bits); val wb_data = out UInt (32 bits);
  ...
  val st = RegInit(IDLE)
  val ma = Reg(UInt(32 bits)); val mw = Reg(UInt(1 bits)); val mr = Reg(UInt(2 bits));
  val mi = Reg(UInt(DCACHE_INDEX bits))
  ...
  when(cap && c0) {
    tagv(ci @@ U(0, 1 bits)) := False; tagv(ci @@ U(1, 1 bits)) := False
  ...
  switch(st) {
    is(IDLE) {
      when(io.req_valid || io.cacop_en) {
        rbv := True; rba := io.cacop_en ? io.cacop_vaddr | io.req_addr
        rbo := io.req_op; rbc := io.cacop_en; rbcm := io.cacop_mode; rbu := io.req_uncached;
        st := LOOKUP
      }
    }
    is(LOOKUP) {
