...
class ChiplabMultiCommitDiffTestAdapterSpec extends AnyFunSuite {
  test("generated wrapper preserves three indexed commits and one global state stream") {
    val outputDirectory = Files.createTempDirectory("openla500-chiplab-multi-difftest-")
    try {
      ...
      assert(rtl.sliding("DifftestExcpEvent u_difftest_exception".length).count(
        _ == "DifftestExcpEvent u_difftest_exception"
      ) == 1)
      assert(rtl.sliding("DifftestCSRRegState u_difftest_csr_state".length).count(
        _ == "DifftestCSRRegState u_difftest_csr_state"
      ) == 1)
      assert(rtl.sliding("DifftestGRegState u_difftest_gpr_state".length).count(
