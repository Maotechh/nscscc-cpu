...
class OpenLa500PerfCounterSpec extends AnyFunSuite {
  test("synchronous reset, independent events, concurrency, and idle hold match the contract") {
    val workspaceRoot =
      sys.env.getOrElse("SPINAL_SIM_WORKSPACE", "target/sim-workspace-openla500-perf-counter")
    val workspace = Paths.get(workspaceRoot, "openla500-perf-counter-contract").toString
  ...
  test("generated legacy wrapper has exactly the locked nine input ports and named counters") {
    val outputDirectory = Files.createTempDirectory("openla500-perf-counter-rtl-")
    try {
