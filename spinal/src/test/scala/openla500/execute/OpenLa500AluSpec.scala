...
class OpenLa500AluSpec extends AnyFunSuite {
  ...
  private def withSimulation(
    ...
    val workspaceRoot =
      sys.env.getOrElse("SPINAL_SIM_WORKSPACE", "target/sim-workspace-openla500-alu")
    val workspace = Paths.get(workspaceRoot, s"openla500-alu-$simulationName").toString
    SimConfig
  ...
  test("generated module has only the native golden ports") {
    val outputDirectory = Files.createTempDirectory("openla500-alu-rtl-")
    try {
