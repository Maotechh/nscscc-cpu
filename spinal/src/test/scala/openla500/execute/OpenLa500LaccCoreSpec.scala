...
class OpenLa500LaccCoreSpec extends AnyFunSuite {
  ...
  test("configuration, lmadd, backpressure, flush, and reset match the golden cycle contract") {
    val workspaceRoot =
      sys.env.getOrElse("SPINAL_SIM_WORKSPACE", "target/sim-workspace-openla500-lacc")
    val workspace = Paths.get(workspaceRoot, "openla500-lacc-cycle-contract").toString
  ...
  test("generated legacy wrapper has exactly the golden LACC ports") {
    val outputDirectory = Files.createTempDirectory("openla500-lacc-rtl-")
    try {
