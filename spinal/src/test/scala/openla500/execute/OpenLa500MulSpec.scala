...
class OpenLa500MulSpec extends AnyFunSuite {
  ...
  test("each active edge captures a full signed or unsigned product and reset holds it") {
    val workspaceRoot =
      sys.env.getOrElse("SPINAL_SIM_WORKSPACE", "target/sim-workspace-openla500-mul")
    val workspace = Paths.get(workspaceRoot, "openla500-mul-cycle-contract").toString
  ...
  test("generated module has exactly the six native golden ports") {
    val outputDirectory = Files.createTempDirectory("openla500-mul-rtl-")
    try {
