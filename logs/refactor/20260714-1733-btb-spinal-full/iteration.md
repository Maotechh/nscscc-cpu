...
- Commands and gate results: 锁定 JDK 17.0.19、SpinalHDL 1.14.2、Verilator 5.020 下 predictor 定向测试 1/1 PASS；pre-commit 与 `508fe52` clean clone 的 `make scala-check` 均为 4/4 task、26/26 ScalaTest PASS，主/测试源码 56/16，skipped=0。`core_top` 两次生成一致，原始 RTL hash `1b9340cd...`；package 合同 49 ports、`TLBNUM=32`、无 legacy backend，打包 hash `07a2a2d9...`。首次完整 `make generate` 在最终 publish-check 正确拒绝旧 tracked package，随后由本提交同步生成物和三份 replacement 清单。机器证据见 `evidence/`。
